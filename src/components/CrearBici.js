...
function CrearBicis() {
  // const [marcoBici, setMarcoBici] = useState();

  const submitCrearBici = (e) => {
    e.preventDefault();
    ...
    console.log('Aqui: '+modeloBici+' '+nombreBici+' '+marcoBici+' '+amortiguadorBici+' '+horquillaBici+' '+cambiadorBici+' '+manetadeCambioBici+' '+ bottomBracketBici +' '+ cassetteBici+' '+cadenaBici+' '+cranckBici+' '+frenosBici+' '+ruedasBici+' '+llantasBici+' '+ejePasanteBici+' '+stemBici+' '+timonBici+' '+asientoBici+' '+dropperAsientoBici);

    // restaura los inputs a vacío
    ...
    <div className="mainCrearBici">
      ...
      <form onSubmit={submitCrearBici}>
        ...
        <div className="formGroup">
          ...
          <div className="unaPreg2">
            <label>Dropper post:</label>
            <textarea type="text" id='dropperAsientoBici' placeholder='Ingrese todo sobre el DROPPER POST de la bici...' />
          </div>
        </div>
        <input type="submit" className="sbmtBICI" value="Submit NUEVA BICI" />
      </form>
    </div>
  );
}
